...
const CourseForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const message = `Hi ${data.name}, thank you for enrolling in the ${data.course} course! Here are your details:\n\nEmail: ${data.email}\nPhone: ${data.number}\n\nTo view our introductory video, click here: [Video_Link_Here]`;
      const whatsappURL = `https://wa.me/9321633596?text=${encodeURIComponent(message)}`;
    ...
    <section>
      <div className="flex flex-col md:flex-row bg-[#F0EDE3] p-6 md:p-12 space-y-6 md:space-y-0">
        ...
        <div className="flex-1 md:pr-8">
          <h2 className="text-3xl tracking-wide font-bold mb-2 md:text-center font-jacques text-[#D8B661]">
            Talk With Us Now!
          </h2>
      ...
      {isSubmitted && (
        <div className="w-[98%] mx-auto bg-green-100 p-4 rounded-lg flex items-center justify-center">
          ...
          <p className="text-green-700 font-montserrat">
            Thank you for enrolling! Check Your WhatsApp for further details.
          </p>
        </div>
      )}
    </section>
